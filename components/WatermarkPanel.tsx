...
const WatermarkPanel: React.FC<WatermarkPanelProps> = ({ config, onConfigChange, isLoading, onApply, onReset }) => {
    ...
    <div className="w-full flex flex-col gap-6 animate-fade-in h-full">
        ...
        <div className="w-full bg-slate-100 rounded-lg p-1 hidden md:flex items-center justify-center gap-1">
            <button onClick={() => onConfigChange(p => ({...p, type: 'text'}))} className={`w-full font-semibold py-2 px-4 rounded-md transition-all duration-200 text-sm flex items-center justify-center gap-2 ${config.type === 'text' ? 'bg-white shadow-sm text-slate-800' : 'text-slate-600 hover:text-slate-800 hover:bg-white/50'}`}><WatermarkTextIcon className="w-5 h-5"/> Text</button>
            <button onClick={() => onConfigChange(p => ({...p, type: 'photo'}))} className={`w-full font-semibold py-2 px-4 rounded-md transition-all duration-200 text-sm flex items-center justify-center gap-2 ${config.type === 'photo' ? 'bg-white shadow-sm text-slate-800' : 'text-slate-600 hover:text-slate-800 hover:bg-white/50'}`}><WatermarkPhotoIcon className="w-5 h-5"/> Photo</button>
        </div>
        ...
        <div className="hidden md:flex items-center gap-2 mt-auto pt-4 border-t border-slate-200">
            ...
            <button onClick={onApply} className="w-full bg-accent-green text-white font-bold py-3 px-6 rounded-lg transition-colors hover:bg-green-600 active:scale-95" disabled={isLoading}>Apply Watermark</button>
        </div>
    </div>
  );
};
